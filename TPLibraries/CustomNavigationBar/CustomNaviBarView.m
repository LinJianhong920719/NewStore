...
@property (nonatomic, readonly) UIButton *m_btnNearRight;
...
@synthesize m_underline = _underline;
@synthesize m_btnLeft = _btnLeft;
@synthesize m_btnNearRight = _btnNearRight;
...
{
    return Rect(ScreenWidth - NaviBarHeight - 2, NaviBarOrignY, [[self class] barBtnSize].width, [[self class] barBtnSize].height);
}

+ (CGRect)nearRightBtnFrame
{
    return Rect(ScreenWidth - NaviBarHeight - 2 - [[self class] barBtnSize].width, NaviBarOrignY, [[self class] barBtnSize].width, [[self class] barBtnSize].height);
}

+ (CGSize)barBtnSize
{
    return Size(NaviBarHeight, NaviBarHeight);
}

+ (CGSize)barSize
{
    return Size(ScreenWidth,(NaviBarOrignY) + NaviBarHeight);
}

+ (CGRect)titleViewFrame
{
    return Rect(65.0f, NaviBarOrignY, ScreenWidth - 130.0f, NaviBarHeight);
}
...
- (void)initUI
{
    ...
    _labelTitle.textColor = RGB_TitleMini;
    _labelTitle.font = UIFontWithName(FontName_Default, FLOAT_TitleSizeNormal);
...
//    _imgViewBg.image = [[UIImage imageNamed:@"NaviBar_Bg"] resizableImageWithCapInsets:UIEdgeInsetsMake(0, 0, 0, 0)];
    ...
    [_underline setFrame:CGRectMake(0, self.frame.size.height, ScreenWidth, 1)];
    [_underline setAlpha:0.5];
    //颜色渐变
    CAGradientLayer *gradient = [CAGradientLayer layer];
    gradient.frame = _underline.bounds;
    gradient.colors = [NSArray arrayWithObjects:(id)[RGB(150, 150, 150) CGColor], (id)[RGBA(255, 255, 255, 1) CGColor], nil];
    [_underline.layer insertSublayer:gradient atIndex:0];
...
- (void)setNearRightBtn:(UIButton *)btn
{
    if (_btnNearRight)
    {
        [_btnNearRight removeFromSuperview];
        _btnNearRight = nil;
    }else{}
    ...
    if (_btnNearRight)
    {
        _btnNearRight.frame = [[self class] nearRightBtnFrame];
        [self addSubview:_btnNearRight];
    }else{}
}
